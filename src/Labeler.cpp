...
class Kmeans {
  ...
  public:
    ...
    int train(float gc_cont, int LONG) {
        ...
        const bool guard = (gc_cont * 100.0F < 35.0F);
        ...
        for (int seed = 0; seed < 50; seed ++) {
            ...
            int nonempty = 0, min_sz = n;
            int_arr sz(k, 0);
            for (int i = 0; i < n; i ++) sz[labels[i]] ++;
            for (int j = 0; j < k; j ++) {
                nonempty += (sz[j] > 0);
                min_sz = std::min(min_sz, sz[j]);
            }
            bool degenerate = guard && ((nonempty < k) || (min_sz < std::max(2, n / 50)));

            /* pick the restart whose best cluster has the highest inits density */
            float den = pos_density();
            bool take = !degenerate && (!have_best || den > best_den * 1.01F);
        ...
        if (!have_best && guard) {
            const int EXTRA = 250;
            for (int seed = 50; seed < 50 + EXTRA; seed ++) {
                kmeans_pp(seed);
                for (int iter = 0; iter < MAX_ITER; iter ++) {
                    predict();
                    flt_arr prev_c = c;
                ...
                int nonempty2 = 0, min_sz2 = n;
                int_arr sz2(k, 0);
                for (int i = 0; i < n; i ++) sz2[labels[i]] ++;
                for (int j = 0; j < k; j ++) {
                    nonempty2 += (sz2[j] > 0);
                    min_sz2 = std::min(min_sz2, sz2[j]);
                }
                if (nonempty2 < k || min_sz2 < std::max(2, n / 50)) continue;
                Debug() << format_log("Warning: using extended restart",
                                      Str(seed) + " den=" + Str((int) (pos_density() * 100.0F)));
                break;
            }
        } else if (!have_best) {
